<script lang="ts">
    ...
    const create = async () => {
        saving = true;
        const res = await fetch(`/${data.info.id}/settings`, {
            method: "PATCH",
            body: JSON.stringify({ name, port, repo, auto, deps, deps_force, build, entry }),
        });
        if (res.ok) {
            await goto("/");
        } else {
    ...
    <div class="grid overflow-hidden h-full">
        <div class="h-full overflow-y-auto px-6 pb-6">
            <div class="card w-full h-max p-4 mb-4 grid grid-cols-[1fr_2fr] items-center gap-y-4 gap-x-6">
                <div class="text-3xl col-span-2">Modify Server {data.info.name} ({data.info.id})</div>

                <InputItem caption="Name" initial={data.info.name} name="name" type="text" bind:value={name} validator={v => v.length > 0 ? null : "Required"} on:valid={valid} on:changed={changed} on:count={count} />
                <InputItem caption="Port" initial={data.info.port} name="port" type="number" bind:value={port} validator={v => v >= 7000 && v <= 7999 ? null : "Must be in 7xxx space"} on:valid={valid} on:changed={changed} on:count={count} />
                <InputItem caption="Repository" initial={data.info.repo} name="repo" type="text" bind:value={repo} validator={v => v.length > 0 ? null : "Required"} on:valid={valid} on:changed={changed} on:count={count} />
            ...
            <div class="card w-full h-max p-4 mb-4 grid grid-cols-[1fr_2fr] items-center gap-y-4 gap-x-6">
                ...
                <InputItem caption="Build Script" initial={data.info.build} name="build" type="text" bind:value={build} validator={() => null} on:valid={valid} on:changed={changed} on:count={count}>
                ...
                <InputItem caption="Entry Point" initial={data.info.path} name="entry" type="text" bind:value={entry} validator={v => v.length > 0 ? null : "Required"} on:valid={valid} on:changed={changed} on:count={count}>
                    <svelte:fragment slot="input" let:input_for let:error let:check>
                        <div class="input-group grid grid-cols-[max-content_1fr] items-center" class:variant-ghost-surface={!error} class:variant-ghost-error={error} >
            ...
            <div class="text-right">
                ...
                {/if}
                <button class="btn variant-filled-surface" class:variant-filled-surface={!ok} class:variant-filled-primary={ok} disabled={!ok || identical || saving} on:click={create}>Save</button>
            </div>
        </div>
    </div>
